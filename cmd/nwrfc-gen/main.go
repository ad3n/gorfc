...
//
// # Codegen for ergonomics vs. codegen for throughput
//
// The default generate mode produces code for ergonomics: typed In /
// Out struct pairs plus a Call helper that routes through
// [nwrfc.Call]. Marshaling still goes through the reflection-based
// path in the core library, so the win is purely type safety — there
// is no measurable performance change versus a hand-written
// nwrfc.Call.
//
// The --fast flag produces code for throughput. It emits a typed,
// reflection-free marshal/unmarshal path: a marshalIn helper builds
// a map[string]any of RFC parameters with direct, statically typed
// field assignments, an unmarshalOut helper reads it back with
// explicit type assertions, and a CallFast wrapper drives the call
// via [nwrfc.CallMap]. No "reflect" import appears anywhere in the
// generated file, so the per-call reflection cost paid inside
// [nwrfc.Call] is eliminated on the hot path. The generated file
// imports only the public nwrfc package — never internal/backend —
// so it compiles unchanged inside an external consumer module. The
// measurable win is performance, not just typing.
//
// Use --fast for the small set of high-volume BAPIs where per-call
// reflection overhead matters. The trade-off is a slightly larger
// generated file and a fast path that only models the fields it was
// generated against. The default (non-fast) output stays completely
// unchanged by this flag and remains the right choice for everything
// else.
package main
...
--fast generates the reflection-free marshal/unmarshal path (CallFast) for
throughput-sensitive callers; the default output is unchanged.
`)
}
...
func runGenerate(args []string) {
	...
	fast := fs.Bool("fast", false, "generate the reflection-free marshal/unmarshal path (throughput-tuned)")
	...
	files, err := generate(*pkgName, desc, *fast)
	if err != nil {
		fail("generate: %v", err)
	}
...
// SourceNote field. The descriptor is embedded so its fields stay at
// the JSON root level, matching the shape of the committed reference
...
// sourceNote returns a stable, deterministic provenance note for the
// given RFC function name. The same name always yields the same
// string, so regenerated descriptors stay byte-stable.
func sourceNote(fn string) string {
	return fmt.Sprintf(
		"Reference descriptor for nexus-spec migration. "+
...
// fastTmpl is the --fast counterpart of sourceTmpl. It emits a
// reflection-free marshal/unmarshal path: marshalIn builds a
// map[string]any of RFC parameters with direct field assignments
// and unmarshalOut reads it back with explicit type assertions. No
// "reflect" import appears anywhere in the output, and the only
// gorfc package imported is the public nwrfc package — never
// internal/backend — so the generated file compiles unchanged
// inside an external consumer module. The win is throughput, not
// just typing.
const fastTmpl = `// Code generated by nwrfc-gen --fast. DO NOT EDIT.
// SPDX-License-Identifier: Apache-2.0
...
// Out contains the EXPORT, TABLES, and CHANGING results for {{.RFCName}}.
type Out struct {
{{- range .FastOut}}
	{{.GoName}} {{.GoType}} ` + "`rfc:\"{{.RFCName}}\"`" + `
{{- end}}
...
// marshalIn converts the typed input struct into a
// map[string]any of RFC parameters. It is reflection-free: every
// field is a direct, statically typed assignment, so the per-call
// reflection cost paid inside nwrfc.Call is eliminated here.
func marshalIn(in In) map[string]any {
	raw := make(map[string]any, {{len .FastIn}})
{{- range .FastIn}}
	raw["{{.RFCName}}"] = in.{{.GoName}}
{{- end}}
	return raw
}

// unmarshalOut reads a map[string]any of RFC parameters into the
// typed output struct. It is reflection-free: every field is read
// through an explicit, statically typed assertion.
func unmarshalOut(raw map[string]any, out *Out) {
{{- range .FastOut}}
	if v, ok := raw["{{.RFCName}}"]; ok {
{{- if eq .GoType "any"}}
		out.{{.GoName}} = v
{{- else}}
...
// CallFast invokes {{.RFCName}} through the reflection-free
// generated path. It is the throughput-tuned sibling of the
// standard generated Call: behavior is identical, but
// marshal/unmarshal avoid the reflection cost in nwrfc.Call.
func CallFast(ctx context.Context, conn *nwrfc.Conn, in In) (Out, error) {
	var out Out
	raw, err := nwrfc.CallMap(ctx, conn, "{{.RFCName}}", marshalIn(in))
	if err != nil {
		return out, err
	}
	unmarshalOut(raw, &out)
	return out, nil
}
`

// fastTestTmpl is the SDK-free test emitted alongside fastTmpl.
const fastTestTmpl = `// Code generated by nwrfc-gen --fast. DO NOT EDIT.
// SPDX-License-Identifier: Apache-2.0
...
{{- end}}
	...
	if _, err := CallFast(context.Background(), conn, In{}); err != nil {
		t.Fatalf("CallFast: %v", err)
	}
}
`
...
type tmplData struct {
	...
	// FastIn / FastOut are the flattened parameter lists used by
	// fastTmpl. FastIn is IMPORT+CHANGING; FastOut is
	// EXPORT+TABLES+CHANGING. The fast path keeps the structs
	// flat (no embedded InOut) so marshalIn / unmarshalOut can be
	// a straight-line list of typed assignments.
	FastIn  []paramDoc
	FastOut []paramDoc
}
...
func generate(pkg string, d backend.FunctionDescriptor, fast bool) (generatedFiles, error) {
	if pkg == "" {
	...
	data.FastIn = append(data.FastIn, data.Imports...)
	data.FastIn = append(data.FastIn, data.Changing...)
	data.FastOut = append(data.FastOut, data.Exports...)
	data.FastOut = append(data.FastOut, data.Tables...)
	data.FastOut = append(data.FastOut, data.Changing...)

	srcTmpl, tstTmpl := sourceTmpl, testTmpl
	if fast {
		srcTmpl, tstTmpl = fastTmpl, fastTestTmpl
	}

	source, err := render(srcTmpl, data)
	if err != nil {
		return generatedFiles{}, err
	}
	test, err := render(tstTmpl, data)
	if err != nil {
		return generatedFiles{}, err
	}
